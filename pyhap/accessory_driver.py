            ...
            for iid, value in new_iid_values.items():
                ...
                if set_result_value is not None:
                    if setter_responses.get(aid, None) is None:
                        logger.warning(
                            "Returning write response '%s' when it wasn't requested for %s %s", set_result_value, aid, iid
                        )
                    had_write_response = True
            ...
            HAP_REPR_CHARS: [
                {
                    ...
                    **({HAP_REPR_VALUE: setter_responses[aid][iid]} if setter_responses.get(aid, {}).get(iid, None) is not None else {})
                }
