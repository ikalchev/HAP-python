...
class HAPServerHandler(BaseHTTPRequestHandler):
   ...
   def _set_encryption_ctx(self, client_public, private_key, public_key, shared_key,
      ...
      @param pre_session_key: The key used during session negotiation (pair verify one and
         two).
   ...
   def _pairing_four(self, client_username, client_ltpk, client_proof, encryption_key):
      ...
      Use it to verify that the client's proof of the private key. Continue to step five.
   ...
   def recv(self, buflen=1042, flags=0):
      ...
      while buflen > 1:
         ...
         if self.curr_in_block is None:
            ...
            self.curr_in_total = struct.unpack("H", block_length_bytes)[0] \
                               + self.in_cipher.tagLength
            self.num_in_recv = 0
            ...
            part = socket.socket.recv(self,
                     min(buflen, self.curr_in_total - self.num_in_recv))
